...
# --- 1. CONFIGURACIÓN ---
st.set_page_config(page_title="Dashboard V37.7 | Grupo Cenoa", layout="wide")
...
def load_all_data():
    ...
    try:
        ...
        m = {
            ...
            'tablero': '% ACUMULADO TABLERO', # Col O
            'final': 'DESEMPEÑO'
        }
        
        # Limpieza de Nombres y Números
        ...
        # Semáforos para dimensiones individuales
        def get_sem(v):
        ...
        # Iniciales para el mapa
        df['Inic'] = df[m['nombre']].apply(lambda x: (x.split()[0][0] + (x.split()[1][0] if len(x.split())>1 else "")).upper() if len(x)>5 else "")
...
if df_raw is not None:
    ...
    with cols_f[0]: f_anio = st.selectbox("AÑO", ["2025", "2026"]) # El año puede filtrar si tuvieras una columna de año
    ...
    # --- PÁGINA: EVOLUCIÓN (DESDE SOLAPA DESEMPEÑO) ---
    if "Evolución" in st.session_state.pagina:
        if f_nom != "Todos":
            # Extraer fila del colaborador
            ...
            valores = []
            for i in range(15, 27): # Índices de las columnas P a AA
                try:
            ...
            prom_real = np.nanmean(valores) if any(v is not None for v in valores) else 0
            ...
            fig_e.add_trace(go.Scatter(
                ...
                text=[f"{v:.0f}%" if v is not None else "" for v in valores],
            ...
            fig_e.update_layout(
                ...
                yaxis=dict(range=[0, 165], dtick=20, title="Alcance Tablero %"),
                margin=dict(l=20, r=20, t=20, b=20)
            )
            st.plotly_chart(fig_e, use_container_width=True)
        else:
            st.info("👈 Selecciona un colaborador en el filtro superior para ver su evolución mensual.")

    # --- PÁGINA: DESEMPEÑO GRAL ---
    elif "Desempeño Gral." in st.session_state.pagina:
        # Lógica de categorías y gráfico de burbujas con iniciales (reutilizando m['final'])
