...
# --- 1. CONFIGURACIÓN ---
st.set_page_config(page_title="Dashboard Grupo Cenoa V49.0", layout="wide")
...
def load_all_data_v49():
    ...
    try:
        ...
        df_des['Inic'] = df_des[m['nombre']].apply(lambda x: (x.split()[0][0] + (x.split()[1][0] if len(x.split())>1 else "")).upper() if len(str(x))>3 else "")

        # --- MAPEO COMERCIAL FLEXIBLE ---
        def process_com(df):
            # Mapeo por posición relativa para evitar el error de "Index out of bounds"
            # C=2, E=4, F=5, G=6, H=7, I=8, AH=33, AI=34
            n_cols = df.shape[1]
            new_cols = {}
            if n_cols > 2: new_cols[df.columns[2]] = 'VENDEDOR'
            if n_cols > 4: new_cols[df.columns[4]] = 'ANTIGUEDAD'
            if n_cols > 5: new_cols[df.columns[5]] = 'EMPRESA'
            if n_cols > 6: new_cols[df.columns[6]] = 'LOCALIDAD'
            if n_cols > 7: new_cols[df.columns[7]] = 'CANAL'
            if n_cols > 8: new_cols[df.columns[8]] = 'OBJETIVO'
            
            # Buscar AH y AI dinámicamente desde el final si el índice fijo falla
            if n_cols >= 35:
                new_cols[df.columns[33]] = 'TOTAL_OPS'
                new_cols[df.columns[34]] = 'PROM_MES'
...
df_raw, df_25, df_26, m = load_all_data_v49()

# --- 4. SIDEBAR ---
with st.sidebar:
    st.title("Grupo Cenoa")
    st.caption("Dashboard 2026 | V49.0")
...
if df_raw is not None:
    ...
    if "Ranking Comercial" in st.session_state.pagina:
        ...
        emp_list = ["Todas"] + sorted(df_c_base['EMPRESA'].dropna().unique().tolist()) if 'EMPRESA' in df_c_base.columns else ["Todas"]
        with f3: f_emp_c = st.selectbox("EMPRESA", emp_list)
    ...
    if "Ranking Comercial" in st.session_state.pagina:
        if 'VENDEDOR' not in df_c_base.columns:
            st.warning("⚠️ No se detectan suficientes columnas en la solapa comercial. Verifica que haya datos hasta la columna AI.")
        else:
            df_r = df_c_base.copy()
            # Mapeo de meses (Indices J=9, L=11, N=13... AH=33)
            m_idx = {"Ene":9,"Feb":11,"Mar":13,"Abr":15,"May":17,"Jun":19,"Jul":21,"Ago":23,"Sep":25,"Oct":27,"Nov":29,"Dic":31,"TOTAL":33}
            col_t = m_idx[f_mes]
            
            if df_r.shape[1] > col_t:
                df_r['Ops'] = pd.to_numeric(df_r.iloc[:, col_t].astype(str).str.replace('-','0').str.replace(',','.'), errors='coerce').fillna(0)
                df_r['Prom_Mes_Val'] = pd.to_numeric(df_r['PROM_MES'].astype(str).str.replace('-','0'), errors='coerce').fillna(0) if 'PROM_MES' in df_r.columns else 0
                ...
                if not top_10.empty:
                    ...
                    cols_ver = ['VENDEDOR', 'EMPRESA', 'ANTIGUEDAD', 'Ops', 'Prom_Mes_Val', m['comp']]
                    st.dataframe(top_10[[c for c in cols_ver if c in top_10.columns]], use_container_width=True)
                else: st.info("Sin datos para los filtros seleccionados.")
            else:
                st.error(f"La columna para el mes {f_mes} no existe en el archivo actual.")
        ...
        st.plotly_chart(px.strip(df_final.dropna(subset=[col_d]), x=m['empresa'], y=col_d, color=m['empresa'], hover_name=m['nombre'], height=550, template="plotly_white"), use_container_width=True)

    elif "Evolución" in st.session_state.pagina:
        if f_nom != "Todos":
            c_data = df_final.iloc[0]
            # Meses en DESEMPEÑO: Col P a AA (Indices 15 a 26)
