...
# --- 1. CONFIGURACIÓN ---
st.set_page_config(page_title="Dashboard Grupo Cenoa V47.0", layout="wide")
...
# --- 2. CSS PREMIUM (Sidebar Bloques + UX) ---
st.markdown("""
    <style>
...
# --- 3. CARGA DE DATOS (RECONSTRUCCIÓN MOTOR) ---
@st.cache_data(ttl=60)
def load_all_data():
    URL = "https://docs.google.com/spreadsheets/d/1fXJ2UsTeOE8ipYXeP5oQYYCHRNtDJDRC/edit"
    try:
        def read_safe(sheet_name):
            p = urllib.parse.quote(sheet_name)
            d = pd.read_csv(f"{URL.split('/edit')[0]}/gviz/tq?tqx=out:csv&sheet={p}")
            return d

        df_des = read_safe("DESEMPEÑO")
        df_p25 = read_safe("PERFO COMERCIAL 2025")
        df_p26 = read_safe("PERFO COMERCIAL 2026")
        ...
        # --- MAPEO COMERCIAL (Columnas C a AI) ---
        def clean_commercial(df):
            # Forzamos nombres a columnas críticas para evitar IndexError
            df = df.iloc[:, :40] # Nos aseguramos de tener hasta la AI
            # C=2, E=4, F=5, G=6, H=7, I=8, J=9(Ene)... AH=33, AI=34
        ...
        return df_des, clean_commercial(df_p25), clean_commercial(df_p26), m
...
with st.sidebar:
    ...
    st.caption("Dashboard 2026 | V47.0")
    ...
    seleccion = st.radio("Nav", menu, index=idx_p, label_visibility="collapsed")
...
if df_raw is not None:
    ...
    # FILTROS GLOBALES
    es_com = "Ranking" in st.session_state.pagina or "Perf" in st.session_state.pagina
    if es_com:
        ...
        with f3: f_emp = st.selectbox("EMPRESA", ["Todas"] + sorted(df_c_base['EMPRESA'].dropna().unique().tolist()))
        with f4: f_vendedor = st.text_input("BUSCAR VENDEDOR...", placeholder="Nombre...")
    else:
    ...
    # --- PÁGINA: RANKING COMERCIAL (BLINDADO) ---
    if st.session_state.pagina == "🥇 Ranking Comercial":
        df_r = df_c_base.copy()
        # Mapeo de meses J=9, L=11, N=13... AH=33
        m_idx = {"Ene":9,"Feb":11,"Mar":13,"Abr":15,"May":17,"Jun":19,"Jul":21,"Ago":23,"Sep":25,"Oct":27,"Nov":29,"Dic":31,"TOTAL":33}
        target = m_idx[f_mes]
        
        df_r['Ops'] = pd.to_numeric(df_r.iloc[:, target].astype(str).str.replace('-','0').str.replace('%',''), errors='coerce').fillna(0)
        df_r['Prom_Mes_Val'] = pd.to_numeric(df_r['PROM_MES'].astype(str).str.replace('-','0'), errors='coerce').fillna(0)
        
        if f_emp != "Todas": df_r = df_r[df_r['EMPRESA'] == f_emp]
        if f_vendedor: df_r = df_r[df_r['VENDEDOR'].str.contains(f_vendedor.upper())]
        ...
        if not top_10.empty:
            ...
            st.dataframe(top_10[['VENDEDOR', 'EMPRESA', 'LOCALIDAD', 'Ops', 'Prom_Mes_Val', m['comp']]], use_container_width=True)
        else: st.warning("Sin datos.")

    # --- PÁGINAS RRHH (RECUPERADAS AL 100%) ---
        ...
        if st.session_state.det_sel == "AUDIT":
            ...
            if st.button("Cerrar Auditoría"): st.session_state.det_sel = None; st.rerun()
        ...
        if f_nom != "Todos":
            ...
            fig_e = go.Figure(go.Scatter(x=["Ene","Feb","Mar","Abr","May","Jun","Jul","Ago","Sep","Oct","Nov","Dic"], y=vals, mode='lines+markers+text', line=dict(color='#3498db', width=4), text=[f"{v:.0f}%" if not np.isnan(v) else "" for v in vals], textposition="top center"))
...
else: st.error("Falla crítica al conectar datos.")
